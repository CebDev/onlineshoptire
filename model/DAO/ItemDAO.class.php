<?php
    ...
    class ItemDAO implements DAO {
        ...
        public static function inserer($unNouveauItem){
            ...
            $isRecorded = $requete->execute(array($unNouveauItem->getModeleNom(), $unNouveauItem->getLargeur(), $unNouveauItem->getRatio(), $unNouveauItem->getDiametre(), $unNouveauItem->getIndiceCharge(), $unNouveauItem->getIndiceVitesse(),  $unNouveauItem->getPrixDeBase(), $unNouveauItem->getStock(), $unNouveauItem->getRemise()));
        ...
        public static function remplirSelectManufacturier(){
            $list = array();
            try{
                $connexion = ConnexionDB::getInstance();
            } catch (Exception $e){
            ...
            $requete = $connexion->prepare("SELECT DISTINCT nom FROM manufacturier ORDER BY nom");
            $requete->execute();
            foreach($requete as $enregistrement){
                array_push($list, $enregistrement);
            }
